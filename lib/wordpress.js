...
export async function getPostsByTag(tagSlug, { page = 1, perPage = 10 } = {}) {
  const { data: tags } = await fetchFromWordPress(
    `${WORDPRESS_API_URL}/tags?slug=${tagSlug}`
  );
  ...
  const { data: posts, response } = await fetchFromWordPress(
    `${WORDPRESS_API_URL}/posts?tags=${tags[0].id}&page=${page}&per_page=${perPage}&_embed=true`
  );
