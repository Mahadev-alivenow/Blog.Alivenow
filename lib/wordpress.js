...
export async function getTrendingPosts(limit = 6) {
  try {
    ...
    const { data: posts } = await fetchFromWordPress(
      `${WORDPRESS_API_URL}/posts?per_page=${limit}&orderby=meta_value_num&meta_key=views&order=desc&_embed=true`
    );
    ...
    try {
      const { data: posts } = await fetchFromWordPress(
        `${WORDPRESS_API_URL}/posts?per_page=${limit}&orderby=comment_count&order=desc&_embed=true`
      );
      return posts.map(formatPost);
    } catch (fallbackError) {
